...
from src.train.dataset import WeChatDataset
from src.train.trainer import FMTrainer
from src.utils import check_path, set_seed, EarlyStopping, collate_fn, get_field_info

from src.model.deepfm import DeepFM
from src.model.pnn import PNN
from src.model.widedeep import WideDeep
from src.model.dcn import DCN
from src.model.autoint import AutoInt
from src.model.xdeepfm import xDeepFM
...
def main():
    ...
    parser.add_argument("--pre_trained_feat", nargs='+', type=str, # 'bgm_song_id', 'bgm_singer_id'
                        default=['userid', 'feedid', 'authorid'])
    ...
    # if args.do_eval:
    #     train_file = f'./data/online_train/online_train_{args.label}_14_generate_sample_{args.rate}.csv'
    # else:
    #
    #     train_file = f'./data/offline_train/offline_train_{args.label}_12_generate_sample_{args.rate}.csv'
    #     test_file = f'./data/evaluate/evaluate_all_13_generate_sample.csv'

    if args.do_eval:
        train_file = os.path.join(PROCESS_DATA_PATH, f'online_{args.label}_generate_sample.csv')
    else:

        train_file = os.path.join(PROCESS_DATA_PATH, f'offline_{args.label}_generate_sample.csv')
    ...
    # PNN
#     if args.label == 'like':
#         no_need_info = ['description_word']
#         args.w2v_window_feed = 5
#         args.w2v_window_author = 50
#     else:
    no_need_info = ['description_char']
    ...
    # save model args
    if args.fold == -1:
        args_str = f'{args.model_name}-{args.label}-{args.batch_size}'
    else:
        args_str = f'{args.model_name}-{args.label}-fold{args.fold}'
    # args_str = f'tune_models'
