...
from src.train.dataset import MultiTaskDataset as WeChatDataset
from src.train.trainer import MultiTrainer as FMTrainer
...
def main():
    ...
    parser.add_argument('--output_dir', default='./data/multitask/', type=str)
    ...
    if args.do_eval:
        train_file = os.path.join(PROCESS_DATA_PATH, f'online_generate_sample.csv')
    else:

        train_file = os.path.join(PROCESS_DATA_PATH, f'offline_generate_sample.csv')
...
#     else:
    ...
    # save model args
    if args.fold == -1:
        args_str = f'{args.model_name}-multi'
    else:
        args_str = f'{args.model_name}-multi-fold{args.fold}'
    ...
    if args.do_eval:
        ...
        checkpoint = 'DCN-multi-first-1024.pt'
        args.checkpoint_path = os.path.join(args.output_dir, checkpoint)
