...
# Pos: min_len:1, max_len:877, avg_len:16.314849075035532.
# Neg: min_len:1, max_len:1168, avg_len:196.13252859535817.
# last=5
# Pos: min_len:1, max_len:521, avg_len:10.553052793459711.
# Neg: min_len:1, max_len:937, avg_len:117.89455732142135.
class WeChatDataset(Dataset):
    def __init__(self, data_file, args, all_field,
        ...
        user_feat = pd.read_csv(os.path.join(PROCESS_DATA_PATH, f'user_feature.csv'), header=0)
        self.data = pd.merge(action_data_df, item_feat, on='feedid', how='left')
        self.data = pd.merge(self.data, user_feat, on=['userid', 'date_'], how='left')
        self.data['feedid'] = self.data['feedid'] + 1
        
        args.logger.info(f'user feature file merged!')
        ...
        self.asr_len = 150
        self.tags_len = 8
        self.feed_seq_len = args.feed_seq_len
        self.feed_seq_len_neg = args.feed_seq_len_neg
        self.phase = phase
    
    def process_feed_seq(self, token_seq, max_len):
    ...
    def get_feature_tensor(self, inter):
        ...
        for feat in self.all_field:
            ...
            elif feat == 'user_his_seq':
                if inter[feat] is np.nan:
                    token_seq = [0]
                else:
                    token_seq = inter[feat].split(' ')
                data[feat] = torch.tensor(self.process_feed_seq(token_seq, self.feed_seq_len), dtype=torch.long)
            elif feat == 'user_his_seq_neg':
                if inter[feat] is np.nan:
                    token_seq = [0]
                else:
                    token_seq = inter[feat].split(' ')
                data[feat] = torch.tensor(self.process_feed_seq(token_seq, self.feed_seq_len_neg), dtype=torch.long)
            
            elif feat == 'description_char':
                if inter[feat] is np.nan:
                    token_seq = [0]
                else:
                    token_seq = inter[feat].split(' ')
