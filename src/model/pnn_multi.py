...
from src.model.layers import BaseFactorizationMachine, MLPLayers, MOE
...
class PNN(ContextRecommender):
    ...
    def __init__(self, args, field_names, field_sources, field2type, field2num):
        ...
        self.predict_layers = MLPLayers(size_list, self.dropout_prob, bn=False)
        
        self.moe = MOE(feature_size=self.mlp_hidden_size[-1], 
                       num_classes=7, dropout_prob=self.dropout_prob)
        self.relu = nn.ReLU()
    ...
    def forward(self, interaction):
        ...
        last_output = torch.cat(output, dim=1)
        outputs = self.predict_layers(last_output)
        logits = self.moe(outputs)
        return logits
    def calculate_loss(self, interaction):
        label = interaction[self.LABEL]
        outputs = self.forward(interaction)
        return outputs
